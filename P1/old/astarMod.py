...
class PacmanAgent(Agent):
    ...
    def max_manhattan_distances(self, curr_pos, food_pos):
        """
        Arguments :
        -----------
        ...
        for i in range(food_pos.width):
            for j in range(food_pos.height):
                if food_pos[i][j]:
                    md = abs(curr_pos[0]-i)+abs(curr_pos[1]-j)
                    if md > max_md:
                        max_md = md
        return max_md
    ...
    def cost(self, curr_p_node, successor):
        ...
        if food_pos[x][y]:
            return curr_p_node.priority + 1 + self.max_manhattan_distances(curr_pos, food_pos)
        else :
            return curr_p_node.priority + 10 + self.max_manhattan_distances(curr_pos, food_pos)
